/**
 * Fast Food Ordering Cart System
 * 
 * This JavaScript file provides cart functionality for a fast food ordering system.
 * It handles adding items to cart, updating quantities, calculating totals,
 * and filtering menu items by category.
 * 
 * @author Your Name
 * @version 1.0
 */
            ...
            cartHTML += `
              <div class="d-flex justify-content-between align-items-center mb-3 pb-3 border-bottom" data-item-id="${id}">
                ...
                <div class="d-flex align-items-center">
                  <button class="btn btn-sm btn-outline-secondary decrement-item" data-id="${id}">-</button>
                  <span class="px-2">${item.quantity}</span>
                  <button class="btn btn-sm btn-outline-secondary increment-item" data-id="${id}">+</button>
    ...
    function initializeMenuListeners() {
      ...
      if (checkoutButton) {
        checkoutButton.addEventListener('click', function() {
          if (cart.count > 0) {
            alert('Proceeding to checkout with ' + cart.count + ' items');
